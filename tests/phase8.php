...
try {
    ...
    $test('Pre-Phase 10 CTA does not link to an unavailable contact route', static function () use ($assert,$router,$slug):void { $body=$router->dispatch(new Request('GET','/klima-uredjaji/'.$slug))->body; $assert(str_contains($body,'id="informacije"') && str_contains($body,'href="/klima-uredjaji"') && !str_contains($body,'href="/kontakt')); });
