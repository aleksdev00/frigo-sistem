...
try {
    ...
    $test('Inquiry CTA preserves trusted product slug context', static function () use ($assert,$router,$slug):void { $assert(str_contains($router->dispatch(new Request('GET','/klima-uredjaji/'.$slug))->body,'/kontakt?proizvod='.$slug)); });
